...
echo local usando tu GPU NVIDIA.
echo.

set "SCRIPT_DIR=%~dp0"
set "LLAMA_DIR=%SCRIPT_DIR%llama"
set "SERVER_EXE=%LLAMA_DIR%\llama-server-cuda.exe"
set "MODEL_DIR=%LLAMA_DIR%\models"

REM ---------- llama-server (CUDA) ----------
if not exist "%SERVER_EXE%" (
    echo.
    echo [Paso 1/3] Descargando instalacion de ~60MB...
    if not exist "%LLAMA_DIR%" mkdir "%LLAMA_DIR%"

    echo    Descargando version con CUDA...

    powershell -Command "$ProgressPreference='SilentlyContinue'; $zip='%LLAMA_DIR%\llama.zip'; Write-Host '   Descargando...'; Invoke-WebRequest -Uri 'https://github.com/ggml-org/llama.cpp/releases/download/b4628/llama-b4628-bin-win-cuda-cu12.4-x64.zip' -OutFile $zip; Expand-Archive -Path $zip -DestinationPath '%LLAMA_DIR%' -Force; Remove-Item $zip"
